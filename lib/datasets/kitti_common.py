...
def get_label_anno(label_path):
    ...
    if len(content) != 0 and len(content[0]) == 114:  # have score
        annotations['score'] = np.array([float(x[15]) for x in content])
        annotations['depth_map'] = np.array([[float(info) for info in x[16:65]] for x in content]).reshape(-1, 49)
        annotations['merge_prob'] = np.array([[float(info) for info in x[65:114]] for x in content]).reshape(-1, 49)
    else:
        annotations['score'] = np.zeros([len(annotations['bbox'])])
        annotations['depth_map'] = np.zeros([len(annotations['bbox']), 49])
        annotations['merge_prob'] = np.zeros([len(annotations['bbox']), 49])
