...
def eval_class_v3(gt_annos,
    ...
    Args:
        ...
        num_parts: int. a parameter for fast calculate algorithm
    Returns:
        dict of recall, precision, aos, thresholds, min_overlaps, valid_dt_no_score
    """
    assert len(gt_annos) == len(dt_annos)
    num_examples = len(gt_annos)
    split_parts = get_split_parts(num_examples, num_parts)

    # overlaps between every (dt, gt) pair, split by parts for speed
    ...
    # NEW: number of valid detections (TP+FP) with *no score threshold* at each IoU
    # shape: [class, difficulty, min_overlap_index]
    valid_dt_no_score = np.zeros([num_class, num_difficulty, num_minoverlap], dtype=np.int64)
            ...
            for k, min_overlap in enumerate(min_overlaps[:, metric, m]):
                ...
                for i in range(len(gt_annos)):
                    tp, fp, fn, similarity, thresholds = compute_statistics_jit(
                        ...
                        metric,
                        min_overlap=min_overlap,
                        thresh=0.0,          # collect at zero threshold
                        compute_fp=False      # just for thresholds
                    )
                ...
                for i in range(len(gt_annos)):
                    tp0, fp0, _, _, _ = compute_statistics_jit(
                        overlaps[i],
                        gt_datas_list[i],
                        dt_datas_list[i],
                        ignored_gts[i],
                        ...
                        metric,
                        min_overlap=min_overlap,
                        thresh=0.0,          # no score cut (include all)
                        compute_fp=True
                    )
                    valid_dt_total += (tp0 + fp0)
                    total_tp += tp0
                    total_fp += fp0
                valid_dt_no_score[m, l, k] = valid_dt_total
                # print(f"[GT COUNT] class={current_class}, diff={difficulty} | valid GT = {total_num_valid_gt}")

                # print(f"[DET COUNT] class={current_class}, diff={difficulty}, IoUThr={min_overlap:.2f} | valid DT (no score cut) = {valid_dt_total} (TP: {total_tp} / FP: {total_fp})")


                pr = np.zeros([len(thresholds), 4])  # [TP, FP, FN, (AOS sum)]
                idx = 0
                for j, num_part in enumerate(split_parts):
                ...
                for i in range(len(thresholds)):
                    precision[m, l, k, i] = pr[i, 0] / (pr[i, 0] + pr[i, 1]) if (pr[i, 0] + pr[i, 1]) > 0 else 0.0
                    # recall[m, l, k, i]   = pr[i, 0] / (pr[i, 0] + pr[i, 2])  # 필요시 사용
                    if compute_aos:
                        aos[m, l, k, i] = pr[i, 3] / (pr[i, 0] + pr[i, 1]) if (pr[i, 0] + pr[i, 1]) > 0 else 0.0

                # precision을 뒤에서부터 non-decreasing 하게 보정
                for i in range(len(thresholds)):
    ...
    ret_dict = {
        "recall": recall,                 # [num_class, num_difficulty, num_minoverlap, 41]
        "precision": precision,           # "
        "orientation": aos,               # "
        "thresholds": all_thresholds,     # [num_class, num_difficulty, num_minoverlap, 41]
        "min_overlaps": min_overlaps,     # [num_minoverlap, metric, num_class]
...
def get_official_eval_result(gt_annos,
    ...
    for j, curcls in enumerate(current_classes):
        ...
        for i in range(min_overlaps.shape[0]):
            ...
            if np.isclose(min_overlaps[i, 2, j], 0.7):
                np.save("monomh_precision_values.npy", metrics["3d"]["precision"][j, :, i])
            ...
            # >>>>>>>>>>> ADD: Rank metrics (TP-only; 논문 스타일) <<<<<<<<<<
            iou_thr_3d = float(min_overlaps[i, 2, j])  # 현재 i의 3D IoU 임계값
            for diff in difficultys:  # 0: easy, 1: moderate, 2: hard
                m_tp = compute_rank_metrics(
                    gt_annos, dt_annos,
                    cls=class_name, difficulty=diff,
                    iou_thr=iou_thr_3d,
                    mode='all',        # 논문처럼 TP만
                    seed=0
                )
                # detail dict에 저장 (JSON 직렬화 가능한 값)
                detail[class_name][f"rank_TP(diff={diff})@3D{iou_thr_3d:.2f}"] = {
                    'pearson':  m_tp['pearson'],
                    'spearman': m_tp['spearman'],
                    'agree':    m_tp['agree'],
                    'n':        m_tp['n'],
                    'pairs':    m_tp['pairs_used'],
                }
                # 텍스트 리포트에도 한 줄 출력하고 싶으면:
                result += print_str(
                    f"rank_TP (diff={diff}) @3D{iou_thr_3d:.2f} | "
                    f"pearson={m_tp['pearson']:.3f}, "
                    f"spearman={m_tp['spearman']:.3f}, "
                    f"agree={m_tp['agree']:.3f} (n={m_tp['n']}, pairs={m_tp['pairs_used']})"
                )
    return {
        "result": result,
        "detail": detail,
...
def eval_from_scrach(gt_dir, det_dir, png_path='', eval_cls_list=None, ap_mode=40):
    global AP_mode
    AP_mode = ap_mode
